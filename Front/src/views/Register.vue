...
async function onSubmit() {
  try {
    ...
    // Adjust API/field names to your backend:
    // Expected backend: POST /api/user/register
    // body: { account, email, role, password }
    ...
    const { data } = resp
    // Assuming your Result<T> shape: { reply: boolean, message: string, data: any }
    if (data?.reply) {
      ElMessage.success(data?.message || 'Registered successfully')
      // redirect to login
